...
def check_if_data_exists(id,type):
    cur=conn.cursor()
    cur.execute("SELECT * FROM github__activities WHERE activity_id=%s AND type=%s",(id,type))
    rows=cur.fetchone()
    if rows:
        return True
    else:
                ...
                if r.status_code==200:
                    for commit in r.json():
                        if check_if_data_exists(commit['sha'],'commit'):
                            break
                        try:
                            curr.execute("INSERT INTO github__activities (repo_name,type,author_id,activity_id,created_at) VALUES (%s,%s,%s,%s,%s)",(repo[0],"commit",commit['author']['id'],commit['sha'],commit['commit']['author']['date']))
                            conn.commit()
                        except Exception as e:
                            conn.rollback()
                            print(e)
                ...
                if r.status_code==200:
                    for pr in r.json():
                        if check_if_data_exists(str(pr['id']),'pull_request'):
                            break
                        try:
                            curr.execute("INSERT INTO github__activities (repo_name,type,author_id,activity_id,created_at) VALUES (%s,%s,%s,%s,%s)",(repo[0],"pull_request",pr['user']['id'],pr['id'],pr['created_at']))
                            conn.commit()
                        except Exception as e:
                            conn.rollback()
                            print(e)
